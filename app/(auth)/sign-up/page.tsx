    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={"space-y-5"}>
                ...
                <InputField name={"email"} label={"Email"} placeholder={"contact@luketaylor.io"} register={register} error={errors.email} validation={{ required: "Email is required", pattern: /^\w+@\w+\.\w+$/, message: "Email address is required" }}/>
                <InputField name={"password"} label={"Password"} placeholder={"Enter a strong password"} register={register} error={errors.password} validation={{ required: "Password is required", minLength: 8 }} type={"password"} />
                ...
                <Button type={"submit"} disabled={isSubmitting} className={"yellow-btn w-full mt-5"}>{isSubmitting ? "Creating Account" : "Start Your Investing Journey"}</Button>
